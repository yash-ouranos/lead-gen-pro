...
export default function MethodOfContactPage() {
  const [methodOfContacts, setMethodOfContacts] = useState<any[]>([]);
  const { filteredData: filteredContacts, filters, setFilters, appliedFilterCount } = useAdvancedFilter(methodOfContacts);
  const [currentPage, setCurrentPage] = useState(1);

  const [itemToDelete, setItemToDelete] = useState<string | null>(null);

  useEffect(() => {
    fetchMethodOfContacts();
  }, []);

  const fetchMethodOfContacts = async () => {
    const res = await fetch("/api/method-of-contact");
    if (res.ok) {
      setMethodOfContacts(await res.json());
    }
  };

  const confirmDelete = async () => {
    if (!itemToDelete) return;
    const res = await fetch(`/api/method-of-contact/${itemToDelete}`, { method: "DELETE" });
    if (res.ok) {
      fetchMethodOfContacts();
    }
  ...
  return (
    <div className="w-full h-full flex flex-col space-y-6">
              ...
              {filteredContacts.slice((currentPage - 1) * 10, currentPage * 10).map((item) => (
                <tr key={item.id} className="bg-card hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-6 py-4 flex gap-3 justify-end">
                    <Link href={`/method-of-contact/${item.id}`} className="text-blue-500 hover:text-blue-700">
      ...
      <ConfirmModal 
        isOpen={!!itemToDelete}
        title="Delete MethodOfContact"
        message="Are you sure you want to delete this methodOfContact? This action cannot be undone."
        confirmText="Delete"
