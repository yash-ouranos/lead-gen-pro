...
export default function MethodOfContactForm({ initialData }: { initialData?: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const url = initialData ? `/api/method-of-contact/${initialData.id}` : "/api/method-of-contact";
    ...
    if (res.ok) {
      ...
      router.push("/method-of-contact");
      router.refresh();
    } else {
      toast.error("Failed to save methodOfContact");
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="flex-1 p-6 md:p-8 space-y-6" noValidate>
      ...
      <div className="flex justify-end gap-3 pt-6">
        <button 
          type="button"
          onClick={() => router.push("/method-of-contact")}
