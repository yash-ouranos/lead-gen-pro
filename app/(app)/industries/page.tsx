...
export default function IndustryPage() {
  const [industries, setIndustrys] = useState<any[]>([]);
  const { filteredData: filteredContacts, filters, setFilters, appliedFilterCount } = useAdvancedFilter(industries);
  ...
  const fetchIndustrys = async () => {
    ...
    if (res.ok) {
      setIndustrys(await res.json());
    }
  };

  const confirmDelete = async () => {
    if (!itemToDelete) return;
    const res = await fetch(`/api/industries/${itemToDelete}`, { method: "DELETE" });
    if (res.ok) {
              ...
              {filteredContacts.slice((currentPage - 1) * 10, currentPage * 10).map((item) => (
                <tr key={item.id} className="bg-card hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-6 py-4 flex gap-3 justify-end">
                    <Link href={`/industries/${item.id}`} className="text-blue-500 hover:text-blue-700">
