...
export default function PermissionsPage() {
  ...
  const { filteredData: filteredPermissions, filters, setFilters, appliedFilterCount } = useAdvancedFilter(permissions);
  ...
  const confirmDelete = async () => {
    ...
    const res = await fetch(`/api/permissions/${itemToDelete}`, { method: "DELETE" });
    if (res.ok) {
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border">
              ...
              {filteredPermissions.slice((currentPage - 1) * 10, currentPage * 10).map((permission) => (
                <tr key={permission.id} className="bg-card hover:bg-muted/50 transition-colors">
                  <td className="px-6 py-4">{permission.name}</td>
                  <td className="px-6 py-4 font-mono text-xs">{permission.value}</td>
                  <td className={`px-6 py-4 font-medium ${permission.status === "Active" ? "text-green-600" : "text-gray-500"}`}>
                    {permission.status}
                  </td>
                  <td className="px-6 py-4">{new Date(permission.createdAt).toLocaleDateString()}</td>
                  <td className="px-6 py-4 flex gap-3 justify-end">
                    <Link href={`/permissions/${permission.id}`} className="text-blue-500 hover:text-blue-700">
                      <Edit className="w-4 h-4" />
                    </Link>
                    <button onClick={() => setItemToDelete(permission.id)} className="text-red-500 hover:text-red-700">
          ...
          <div className="sticky bottom-0 z-10 px-6 py-4 border-t flex items-center justify-between bg-card shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            <p className="text-xs text-muted-foreground font-medium">
              ...
              <span className="font-bold text-foreground">
                {Math.min(currentPage * 10, filteredPermissions.length)}
              </span>{""}
              of <span className="font-bold text-foreground">{filteredPermissions.length}</span> entries
            </p>
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-muted-foreground font-bold px-3 uppercase tracking-wider">
                Page {currentPage} of {Math.max(1, Math.ceil(filteredPermissions.length / 10))}
              </span>
              <button
                onClick={() => setCurrentPage(p => Math.min(Math.ceil(filteredPermissions.length / 10), p + 1))}
                disabled={currentPage >= Math.ceil(filteredPermissions.length / 10)}
                className="p-1.5 border border-border text-foreground hover:bg-muted disabled:opacity-50 disabled:hover:bg-transparent transition-colors"
              >
